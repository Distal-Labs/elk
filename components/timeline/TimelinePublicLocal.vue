...
const paginator = useMastoClient().v1.timelines.listPublic({ limit: 15, local: true })
...
<template>
  <template v-if="isHydrated">
    <div>
      <TimelinePaginator v-bind="{ paginator, stream }" :preprocess="preprocessGlobalFeed" context="public" />
    </div>
  </template>
</template>
