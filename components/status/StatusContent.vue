...
const { status, context, inNotification, isCompact } = defineProps<{
...
const { statuses } = useSearch(statusQuery)
const linkedStatus = computed(() => {
  if (statusQuery.value.length === 0)
    return undefined
...
if (status.card && linkToStatus)
  statusQuery.value = linkToStatus?.toString()
